...
type TrainingProgress = Record<string, { completed: boolean; score: number; xpEarned: number }>
...
export default function TrainingPage() {
  ...
  useEffect(() => {
    setUser(getItem<User>("user", SEED_USER))
    setProgress(getItem<TrainingProgress>("training_progress", {}))
  }, [])

  const currentLevelXp = levelThresholds[user.level - 1] ?? 0
  const nextLevelXp =
    levelThresholds[user.level] ?? levelThresholds[levelThresholds.length - 1]
    ...
    <div>
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500">
            Roleplay scenarios with AI prospects. Earn XP and level up.
          </p>
        </div>
      </div>
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-xl p-5 mb-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-lg font-bold text-blue-900">{user.levelName}</p>
            </div>
          </div>
          <div className="text-right">
            <p className="text-xl font-bold text-blue-700">{user.xp}</p>
        ...
        <div className="flex justify-between mt-1.5">
          <span className="text-xs text-blue-500">{user.levelName}</span>
          <span className="text-xs text-blue-500">
            {nextLevelXp - user.xp} XP to{" "}
            {levelNames[user.level] ?? "Max Level"}
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-4 text-center">
          ...
          <p className="text-xs text-slate-500 mt-0.5">Scenarios</p>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-4 text-center">
          <p className="text-2xl font-bold text-slate-900">{totalXpEarned}</p>
      ...
      <div className="space-y-3">
        {TRAINING_SCENARIOS.map((scenario) => {
          ...
          return (
            <Link
              key={scenario.id}
              href={`/training/session/${scenario.id}`}
      ...
      <div className="mt-8 bg-white border border-slate-200 rounded-xl p-5">
        ...
        <div className="flex items-center justify-between">
          {levelNames.map((name, i) => {
            ...
            const reached = user.xp >= xpNeeded
