...
import { SEED_USER, SEED_HISTORY, SEED_KNOWLEDGE, type HistoryEntry } from "@/lib/seedData"
...
export default function Dashboard() {
  ...
  useEffect(() => {
    // Seed on first visit
    if (!getItem("seeded", false)) {
      setItem("knowledge_base", SEED_KNOWLEDGE)
      setItem("history", SEED_HISTORY)
      setItem("user", SEED_USER)
      setItem("training_progress", {})
      setItem("seeded", true)
    }

    const userData = getItem<User>("user", SEED_USER)
    setUser(userData)

    const history = getItem<HistoryEntry[]>("history", [])
    const analyses = history.filter((h) => h.type === "copilot").length
    const brainstorms = history.filter((h) => h.type === "brainstorm").length
    const trainingSessions = history.filter((h) => h.type === "training")
    const avgScore =
      trainingSessions.length > 0
        ? Math.round(
            trainingSessions.reduce((a, b) => a + (b.score ?? 0), 0) /
              trainingSessions.length
          )
        : 0

    setStats({ analyses, avgScore, brainstorms })
    setRecentActivity([...history].reverse().slice(0, 3))
  }, [])
    ...
    <div>
      ...
      <div className="mb-8">
        ...
        <p className="text-slate-500 mt-1">
          Continue your journey from representative to consultant.
        </p>
      </div>
        ...
        {[
          {
            ...
            desc: "Paste a transcript for instant insights",
            icon: Mic2,
          ...
          },
          {
            ...
            desc: "Generate objection strategies",
            icon: Lightbulb,
          ...
          },
          {
            ...
            desc: "Earn XP with roleplay sessions",
            icon: GraduationCap,
          ...
          },
          {
            ...
            desc: "Browse senior team insights",
            icon: BookOpen,
          ...
          return (
            <Link
              key={action.href}
              href={action.href}
              className={`group flex items-center gap-4 bg-white border border-slate-200 ${action.border} rounded-xl p-4 transition-all`}
            >
              <div
                className={`w-10 h-10 rounded-lg ${action.bg} flex items-center justify-center transition-colors flex-shrink-0`}
              >
      ...
      {recentActivity.length === 0 ? (
        <div className="bg-white border border-slate-200 rounded-xl p-10 text-center">
          ...
          <p className="text-slate-400 text-xs mt-1">
            Start by analyzing a meeting or trying a training scenario.
          </p>
        </div>
      ) : (
        <div className="space-y-2">
          {recentActivity.map((item) => (
            <Link
              key={item.id}
              href="/history"
              className="flex items-center gap-4 bg-white border border-slate-200 hover:border-slate-300 rounded-xl p-4 transition-colors"
            >
              <div
